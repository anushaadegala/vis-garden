//alert($(parent.document).find('#wayExport').prop('checked'));
var xScale;
var yScale;
...
d3.csv("file/consolidated_v3.csv",function(data) {
...
//update on selection changes------------------------------------
	...
	//$(parent.document).find('#wayExport').prop('checked');
	$(parent.document).find('.myInput').on("change", update);
    ...
    function update() {
        ...
        for (var i=3; i<7; i++) {
			$(parent.document).find('#wayExport').prop('checked');
            if ($(parent.document).find("#year201"+i).prop('checked')) {
                years.push(i);
                pieYears = years;
            }
        } 
        ...
        if ($(parent.document).find('#wayImport').prop('checked')) {
            trade = "im";
            }
        if ($(parent.document).find('#wayExport').prop('checked')) {
            trade = "ex";
        }
        
        
        if ($(parent.document).find('#wayCountry').prop('checked')) {
            category = sumByCountry;
            pieCategory = stateSumByCountry;
            xLabel = "Country";
        }
        
        if ($(parent.document).find('#wayCommodity').prop('checked') && trade==="im") {
        ...
        if ($(parent.document).find('#wayCommodity').prop('checked') && trade==="ex") {
